...
struct Client {
  ...
  unsigned int switchtotag;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow, issticky, isfakefullscreen;
	pid_t pid;
	Client *next;
	Client *snext;
	Client *swallowing;
...
prevtiled(Client *c, Monitor *m) {
	...
	for(p = selmon->cl->clients, r = NULL; p && p != c; p = p->next)
		if(!p->isfloating && ISVISIBLE(p, c->mon))
			r = p;
	return r;
}
...
{
	Client *sel = selmon->sel, *c= selmon->sel;

	if(!sel || sel->isfloating || sel == nexttiled(selmon->cl->clients, c->mon))
		return;
	if((c = nexttiled(sel->next, c->mon))) {
		detach(sel);
		sel->next = c->next;
		c->next = sel;
	}
...
pushup(const Arg *arg)
{
	...
	if(!sel || sel->isfloating)
		return;
	if((c = prevtiled(sel, c->mon)) && c != nexttiled(selmon->cl->clients, c->mon)) {
		detach(sel);
		sel->next = c;
...
zoom(const Arg *arg)
{
	...
	if (!selmon->lt[selmon->sellt]->arrange || !c || c->isfloating)
		return;
	if (c == nexttiled(selmon->cl->clients, c->mon)) {
		at = findbefore(prevzoom);
		if (at)
			cprevious = nexttiled(at->next, c->mon);
		if (!cprevious || cprevious != prevzoom) {
			prevzoom = NULL;
			if (!c || !(c = nexttiled(c->next, c->mon)))
				return;
		} else
			c = cprevious;
	}
